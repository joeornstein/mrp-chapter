...
compare_to_truth <- function(estimates, truth){
  ...
  labs(x = 'Estimate',
       y = 'Truth',
       caption = paste0('Mean Absolute Error = ', round(mean(abs(d$estimate - d$truth)), 2)))
}
...
A better approach is to estimate a multilevel model (alternatively known as "varying-intercepts" or "random effects" model), including group-level covariates ; adding Biden vote share
...
model3 <- glmer(defund_police ~  gender + educ + race + age +
                (1|region) + biden_vote_share, 
                 data = sample_data,
                 family = 'binomial')

# construct the poststratification frame
psframe <- ces %>% 
  count(abb, gender, educ, race, age, biden_vote_share)
...
First, be wary of estimates that overshrink towards the grand mean. As we saw in Figure 2, when the first-stage model is [^4] This highlights one of the dangers of producing MRP estimates from an underspecified model. When the first-stage model is underfit, poststratified estimates tend to overshrink towards the global mean. This compression means we should be wary of MRP studies that show policy outcomes "leapfrogging" estimated public opinion [@simonovits2020]. It could be that policymakers are more extreme than their constituents, or that MRP produces estimates of constituency preferences that are too moderate.

[^4]: In the limit, a first-stage model with zero predictors would yield identical poststratified estimates for each state, equal to the survey sample mean.
